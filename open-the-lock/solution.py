...
class Solution(object):
    ...
    def openLock(self, deadends, target):
        ...
        while True:
            if not nodes_s:
                return -1
            nodes_in_next_step = set()
            while nodes_s:
                node = nodes_s.pop()
                steps = visited_from_source[node]

                for node in self.nextNodes(node, deadends):
                    if node in visited_from_target:
                        return steps + visited_from_target[node] + 1
                    if node in visited_from_source:
                        continue
                    visited_from_source[node] = steps+1
                    nodes_in_next_step.add(node)
            nodes_s = nodes_in_next_step
            ...
            while nodes_d:
                node = nodes_d.pop()
                steps = visited_from_target[node]

                for node in self.nextNodes(node, deadends):
                    if node in visited_from_source:
                        return steps + visited_from_source[node] + 1
                    if node in visited_from_target:
                        continue
                    visited_from_target[node] = steps+1
                    nodes_in_next_step.add(node)
            nodes_d = nodes_in_next_step
